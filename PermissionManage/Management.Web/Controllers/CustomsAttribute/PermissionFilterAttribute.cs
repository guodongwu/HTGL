    ...
    public class PermissionFilterAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            if (this.AuthorizeCore(filterContext) == false)//根据验证判断进行处理
            {
                ...
                {
                    //跳转到登录页面
                    filterContext.RequestContext.HttpContext.Response.Redirect("~/Manage/UserLogin");
                }
                else
                {
                    object[] attrsUIException = filterContext.ActionDescriptor.GetCustomAttributes(typeof(UIExceptionResultAttribute), true);
                    if (attrsUIException.Length == 1)
                    {
                        filterContext.Result = new ContentResult() { Content = "您没有权限执行此操作！" };//功能权限弹出提示框
                    }
                    else
                        filterContext.RequestContext.HttpContext.Response.Redirect("~/Manage/Error");
                }
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// [LoginAllowView标记]验证是否登录就可以访问(如果已经登陆,那么对于标识了LoginAllowView的方法就不需要验证了)
        /// </summary>
        /// <param name="filterContext"></param>
        /// <returns></returns>
        ...
        protected virtual bool AuthorizeCore(ActionExecutingContext filterContext)
        {
            ...
            //检查当前角色组有没有超级角色
            if (ToolsHelper.CheckStringHasValue(CurrentUser.UserRoles, ',', AdminUserRoleID))
            {
                return true;
            }
