...
namespace HTGL.Repository.EF
{
    ...
    {
        protected DbContext Context = DataContextFactory.GetCurrentDbContext();
        #region 属性字段
        /// <summary>
        ///     获取当前实体的查询数据集
        /// </summary>
        ...
        {
            totalCount = Context.Set<TEntity>().Where(predicate).Count();
            return Context.Set<TEntity>().Where(predicate).Skip((pageIndex - 1) * pageSize).Take(pageSize).Select(s => s);
        ...
        public IQueryable<TEntity> FindPageList<TOrder>(Expression<Func<TEntity, bool>> predicate, Expression<Func<TEntity, TOrder>> order, bool isAsc, int pageSize, int pageIndex, out int totalCount)
        {
            ...
            {
                return Context.Set<TEntity>().Where(predicate)
                         .OrderByDescending<TEntity, TOrder>(order)
                         .Skip(pageSize * (pageIndex - 1))
                         .Take(pageSize);
            }
