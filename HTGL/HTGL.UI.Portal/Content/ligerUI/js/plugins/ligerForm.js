...
* 修改初始化ligerform部分。增加field属性，用户弹出框的新增和编辑保存
*/
(function ($) {
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
            ...
            if (p.fields && p.fields.length) {
                ...
                $(p.fields).each(function (index, field) {
                    ...
                    if (field.type == "hidden") {
                        out.push('<input type="hidden" id="' + name + '" name="' + name + '" field="' + name + '" />');
                        return;
                    }
        ...
        _buliderControl: function (field) {
            ...
            if (field.comboboxName && field.type == "select") {
                out.push('<input type="hidden" id="' + p.prefixID + name + '" name="' + name + '" field="' + name + '" />');
            }
            ...
            if (field.comboboxName && field.type == "select") {
                //添加特殊字段，用于form表单处理
                out.push('name="' + field.comboboxName + '" field="' + field.comboboxName + '"');
                if (p.appendID) {
